...
class BedFightPendingSession {
    ...
    public function connect(string $name): void {
        ...
        if ($this->countConnections() > 2) {
            ...
            if ($arenaData !== null) {
                $helper->BedFightGameManager()->startGame($player1,$player2,$arenaName, $arenaData);
                $this->disconnect($player1->getName());
                $this->disconnect($player2->getName());
            }
        }
